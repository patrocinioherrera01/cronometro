...
export class AppComponent implements OnInit {
  ...
  agregarColumnas(){
    ...
    this.candidatos.push({id: index+1, nombre: " ", tiempo: 0, minutos: 0, segundos: 59});
  ...
  agregaTiempo(){
    ...
    for (let index = 0; index < this.cantidad; index++) {  
    this.candidatos[index].minutos  = this.objCandidato.minutos -1;
    //this.candidatos[index].segundos = this.objCandidato.segundos;
    }
  }
  ...
  temporizar(){
    ...
    if (--this.candidatoSelec.segundos < 0){
      this.segundos=59;
    if (--this.candidatoSelec.minutos < 0){
      this.resetartiempo();
    }
  }
